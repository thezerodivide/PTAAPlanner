...
void SpendAAFromINI()
{
	...
	for (const AAInfo& info : vAAList)
	{
		...
		// If the player already owns a rank in this AA line, the next linked
		// ability is the rank that should actually be purchased.
		 ...
		 *
		 * On Triune, CurrentRank and MaxRank describe the AA record itself,
		 * not necessarily the number of ranks the character currently owns.
		 * For example, an unowned 1-rank AA may report CurrentRank=1 and
		 * MaxRank=1 while CanTrainAbility() correctly returns true.
		 *
		 * Therefore CanTrainAbility() is the authoritative test for whether
		 * the candidate rank is purchasable. Do not use CurrentRank == MaxRank
		 * as a "maxed" check here.
		 */
		...
		// pAbility now represents the rank we are about to purchase, so the
		// currently-owned rank is one less than the candidate rank.
		int ownedLevel = purchaseLevel > 0 ? purchaseLevel - 1 : 0;

		if (bDebug)
		{
			WriteChatf("MQ2AASpend :: %s is \agavailable\ax for purchase. Cost is %d, tab is %d",
				vRef.c_str(), aaCost, aaType);
			WriteChatf("MQ2AASpend :: %s current level is %d; next purchase is level %d",
				vRef.c_str(), ownedLevel, purchaseLevel);
		}

		// Respect a numeric maximum rank from the INI. "M" means purchase
		// every rank that CanTrainAbility() reports as available.
		...
		// Purchase one AA per pass, matching the original plugin behavior.
		break;
	}
}
